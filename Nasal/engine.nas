# Engine Systems simulation for the Cessna 140, based on Cessna 172P
#
...
setprop("/engines/engine[0]/low-oil-temperature-factor", low_oil_temperature_factor);
		...
		if (getprop("/controls/engines/engine[0]") == 0)
			setprop("/engines/engine[0]/oil-level", 7);
		if (getprop("/controls/engines/engine[0]") == 1)
...
var carb_icing_function = maketimer(1.0, func {
	if (getprop("/engines/engine[0]/carb_icing_allowed")) {
		...
		if (getprop("/controls/engines/engine/carburetor-heat"))
...
controls.incThrottle = func {
	...
	var old_value = getprop("/controls/engines/engine/throttle");
	var new_value = std.max(0.0, std.min(old_value + delta, 1.0));
	setprop("/controls/engines/engine/throttle", new_value);
};
...
setlistener("/controls/engines/mixture-all", func{
	...
	var new_value = std.max(0.0, std.min(value, 1.0));
	setprop("/controls/engines/engine/mixture", new_value);
}, 0, 0);
...
controls.adjMixture = func {
	...
	var old_value = getprop("/controls/engines/engine/mixture");
	var new_value = std.max(0.0, std.min(old_value + delta, 1.0));
	setprop("/controls/engines/engine/mixture", new_value);
};
...
var _axisMode = {
	0: controls.perIndexAxisHandler("/controls/engines/engine[","]/throttle"),
	1: controls.perIndexAxisHandler("/controls/engines/engine[","]/mixture"),
	2: controls.perIndexAxisHandler("/controls/engines/engine[","]/propeller-pitch")
};
